...
class MeterServiceTest {
    ...
    void testGetAllMeterNumbers() {
        List<MeterTest> meters = new ArrayList<>();
        meters.add(meter);
        ...
        // Call the method to be tested
        List<MeterTest> result = meterService.getAllMeterNumbers();
    ...
    void testGetMeterById() {
        Optional<MeterTest> optional = Optional.of(meter);

        // Mocking the meterRepository method
        when(meterRepository.findById(1)).thenReturn(optional);

        // Call the method to be tested
        Optional<MeterTest> result = meterService.getMeterById(1);
